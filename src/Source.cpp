		...
		if (iter) {
			...
			do {
				...
				if (iter->BoundingBox(tesseract::RIL_PARA, &left, &top, &right, &bottom)) {
					...
					duck.addBlock(text, bbox);
				}
			} while (iter->Next(tesseract::RIL_PARA));

			std::cout << "Match confidence " << duck.getMatchConfidence() << " Strings:\n";
			if (duck.getMatchConfidence() < 0.1) {
				backoff.sleep();
			} else {
				backoff.mark();
			}
			if (duck.getMatchConfidence() < 0.3) {
				continue;
			}
			for (const auto &match : duck.matches) {
